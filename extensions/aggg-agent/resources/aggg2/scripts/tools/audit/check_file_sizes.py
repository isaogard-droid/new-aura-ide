...
"""Единый гейт размера файлов AGGG2.0 (god-файлы запрещены).
...
- хук прошивки harness/hooks/aggg2_prompt_hook.py — КОНСТАНТЫ-ЗЕРКАЛО
...
Запуск:
    python3 scripts/tools/audit/check_file_sizes.py       # отчёт (exit 0)
    python3 scripts/tools/audit/check_file_sizes.py --ci  # гейт: exit 1 при error
"""
import argparse
import json
import os
...
EXCLUDE_DIRS = {".git", "db", "node_modules", "__pycache__", ".cache",
                "dist", "build", "vendor", ".venv", "CHANGELOG",
                "quarantine-skills"}

# Имена, которые не считаются (append-only журналы, генерируемое, бэкапы).
EXCLUDE_NAMES = {"CHANGELOG.md", "index.md", "log.md"}

BASELINE_PATH = Path(__file__).resolve().parents[2] / "file_size_baseline.json"
...
def main():
    ...
    ap.add_argument("--root", default=None,
                    help="корень для сканирования (по умолчанию — корень "
                         "AGGG2.0; для pre-commit хука — git rev-parse "
                         "--show-toplevel)")
