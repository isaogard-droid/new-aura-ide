...
(docs/canon/FILE-SIZE.md). build.py реэкспортирует extract_* (контракт тестов)."""
import ast
import glob
import os
import sys
...
def _ts_parser(content, ext):
    ...
    живёт в venv проекта (~/.venvs/aggg2, приходит с code-review-graph), а
...
def _venv_site_packages():
    """site-packages venv проекта (~/.venvs/aggg2) или None."""
    if os.name == "nt":
        p = os.path.join(os.path.expanduser("~"), ".venvs", "aggg2",
                         "Lib", "site-packages")
