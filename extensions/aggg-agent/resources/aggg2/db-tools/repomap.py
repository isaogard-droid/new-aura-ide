...
Использование:
    python3 db-tools/repomap.py project --db db/aggg2.db --tokens 1500
    python3 db-tools/repomap.py project --focus db-tools/search.py
    python3 db-tools/repomap.py file db-tools/search.py --db db/aggg2.db
"""
import argparse
import contextlib
import os
...
DEFAULT_DB = Path(__file__).resolve().parent.parent / "db" / "aggg2.db"
...
def main() -> int:
    ...
    if len(sys.argv) > 1 and sys.argv[1] not in ("project", "file",
                                                  "-h", "--help"):
        # did-you-mean: частая ошибка — «repomap.py db/foo.db» вместо
        # сабкоманды project/file (паттерн search.py _did_you_mean)
        import difflib
        guess = difflib.get_close_matches(sys.argv[1],
                                          ["project", "file"], n=1)
        hint = f" — возможно, ты имел в виду '{guess[0]}'?" if guess else ""
        print(f"ошибка: '{sys.argv[1]}' — не сабкоманда. Использование:\n"
              f"  repomap.py project --db db/<имя>.db --tokens 1500\n"
              f"  repomap.py file <rel_path> --db db/<имя>.db{hint}")
        return 2
    args = ap.parse_args()
    with contextlib.suppress(Exception):
        from fresh import ensure as _ensure_fresh
        _ensure_fresh(Path(args.db).name.replace('.db', ''))
    # Телеметрия: карта/свежесть — в search_log (аудит 17.08.2026,
    # research.db id=791: «базой пользуемся редко» нечем было замерить;
    # search.py логируется, repomap — нет).
    with contextlib.suppress(Exception):
        from log import log_search
        log_search("repomap.py",
                   os.path.basename(args.db).replace(".db", ""),
                   f"{args.what}:{getattr(args, 'path', '') or ','.join(getattr(args, 'focus', None) or [])}",
                   1)
