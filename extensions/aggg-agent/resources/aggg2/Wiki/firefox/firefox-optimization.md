...
description: "Optimize Firefox on Fedora/Linux for performance and stability ('не глючит, но тянет'): about:config tweaks via user.js (Betterfox/Fastfox verified list), hardware acceleration, We"
date: 2026-08-16
tags: [skill-notes, firefox, performance, about-config, user-js]
source: skills/firefox-optimization/ (перенесено 16.08.2026)
status: stable
---

# firefox-optimization — скилл-на-полке (Howto)

Перенесено из `skills/firefox-optimization/` по протоколу `docs/canon/WIKI.md` (скилл-на-полке: узкий скилл, не в общем пуле). Полная инструкция ниже — дословно.
...
Goal: make Firefox fast and stable — "не глючит, но тянет". Sources: Betterfox
