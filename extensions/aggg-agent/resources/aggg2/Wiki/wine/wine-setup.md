...
type: Howto
title: wine-setup
description: "Full Wine setup on Fedora for Windows games and repacks: install wine/winetricks + missing components (MFC42, VC++ runtimes, DirectX, corefonts, d3dx), map a folder as an extra dri"
date: 2026-08-16
tags: [skill-notes, wine, setup, windows-games, winetricks]
source: skills/wine-setup/ (перенесено 16.08.2026)
status: stable
---

# wine-setup — скилл-на-полке (Howto)

Перенесено из `skills/wine-setup/` по протоколу `docs/canon/WIKI.md` (скилл-на-полке: узкий скилл, не в общем пуле). Полная инструкция ниже — дословно.
...
```bash
cd ~/.wine/dosdevices
ln -s <путь-к-диску> d:   # например /run/media/$USER/DATA      # install games to D:\Games\...
```
...
- Multiple data folders: add more (e: → <ещё-одна-папка>, etc.).
...
The installer silently dies → "setup.exe не запускается/ничего не происходит".
Fix:
