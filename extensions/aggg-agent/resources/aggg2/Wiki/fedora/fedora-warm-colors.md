---
type: Howto
title: fedora-warm-colors
description: "Make a Fedora GNOME (Wayland) display look warm and pleasant like Windows instead of cold/washed out. Use when the user asks to fix cold colors, set up gamma, night light, color te"
date: 2026-08-16
tags: [skill-notes, fedora, gnome, display, colors]
source: skills/fedora-warm-colors/ (перенесено 16.08.2026)
status: stable
---

# fedora-warm-colors — скилл-на-полке (Howto)

Перенесено из `skills/fedora-warm-colors/` по протоколу `docs/canon/WIKI.md` (скилл-на-полке: узкий скилл, не в общем пуле). Полная инструкция ниже — дословно.
...
echo "== Extensions (expect Состояние: ACTIVE for all 4)"; \
for u in dash-to-panel@jderose9.github.com arcmenu@arcmenu.com blur-my-shell@aunetx ding@rastersoft.com; do \
  echo "$u: $(gnome-extensions info "$u" 2>/dev/null | grep -i -E 'state|состоя' | head -1)"; done
...
- Windows-look rollbacks (cursors, sounds, extensions, dark theme, sudo) —
  `references/windows-look-extras-2.md` §Rollback (вынесено из
  `windows-look-extras.md` при резке god-файла).
